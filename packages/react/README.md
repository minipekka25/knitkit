...
The thin React client wrapper for **knitkit** — load a federated remote component by name, with Suspense and an error boundary already wired up.
...
The remote's exposed module must **default-export a React component**. Register your remotes once (browser: `registerRemotes` from `@knitkit/runtime`; SSR: `registerFederation` from `@knitkit/node`) before the component mounts.
...
own bootstrap or use an iframe; for edge-side stream stitching see [`@knitkit/edge`](../edge/README.md).
...
- This is the RSC **Tier 1** surface: the runtime works *inside* RSC apps (server components load via `@knitkit/node`, client components via the browser path behind this wrapper). Fragment-level and module-level RSC federation are later tiers — see the project roadmap.
