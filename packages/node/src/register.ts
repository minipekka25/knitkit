...
// Side-effect entry for the `node --import @knitkit/node/register` workflow.
...
function loadMapFromEnv(): ImportMap | undefined {
  ...
  try {
    ...
    process.emitWarning(
      `[knitkit] @knitkit/node/register: could not parse import map from env: ${(e as Error).message}`,
    );
    return undefined;
  }
  return undefined;
...
if (map) {
  ...
  process.emitWarning(
    "[knitkit] @knitkit/node/register loaded but no import map found. " +
